...
$cart_bundle_items = $this->crud_model->getCartBundleItems();
?>
<div class="icon">
    <a href="<?php echo site_url('home/my_wishlist'); ?>"><i class="far fa-heart"></i></a>
    <span class="number"><?php echo sizeof($this->crud_model->getWishLists()) + sizeof($this->crud_model->getBundleWishLists()); ?></span>
</div>
    ...
    <div class="list-wrapper">
        <div class="item-list">
            <ul>
                ...
                <?php endforeach; ?>
                <?php
                foreach (json_decode($user_details['wishlist_bundle']) as $wishlist) :
                    $wishlist_bundle_details = $this->course_bundle_model->get_bundle_details($wishlist)->row_array();
                    $instructor_details = $this->user_model->get_all_user($wishlist_bundle_details['user_id'])->row_array();
                ?>
                    <li>
                        <div class="item clearfix">
                            <div class="item-image">
                            ...
                            <div class="item-details">
                                <a href="<?php echo site_url('home/course/' . rawurlencode(slugify($wishlist_bundle_details['title'])) . '/' . $wishlist_bundle_details['id']); ?>">
                                    <div class="course-name"><?php echo $wishlist_bundle_details['title']; ?></div>
                                    <div class="instructor-name">
                                        <?php echo site_phrase('by') . ' ' . $instructor_details['first_name'] . ' ' . $instructor_details['last_name']; ?>
                                    </div>

                                    <div class="item-price">
                                        <span class="current-price"><?php echo currency($wishlist_bundle_details['price']); ?></span>
                                        <span class="original-price"><?php echo $this->course_bundle_model->get_original_price_of_bundle($wishlist_bundle_details['id']); ?></span>
                                    </div>
                                </a>
                                <?php if ($this->course_bundle_model->is_bundle_purchased($this->session->userdata('user_id'), $wishlist_bundle_details['id'])) : ?>
                                    <button><?php site_phrase('already_purchased'); ?></button>
                                <?php else : ?>
                                        <button type="button" id="<?php echo $wishlist_bundle_details['id']; ?>" onclick="handleBundleCartItems(this)" class="<?php if (in_array($wishlist_bundle_details['id'], $cart_bundle_items)) echo 'addedToCart'; ?>">
                                            <?php
                                            if (in_array($wishlist_bundle_details['id'], $cart_bundle_items)) {
                                                echo site_phrase('added_to_cart');
                                            } else {
                                                echo site_phrase('add_to_cart');
                                            }
