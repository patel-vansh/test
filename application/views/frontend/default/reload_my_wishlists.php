        ...
        <div class="course-box-wrap">
            <div class="course-box">
                <div class="course-image">
                    ...
                    <div class="wishlist-add wishlisted">
                        <button type="button" onclick="handleBundleWishList(this)" id="<?php echo $my_bundle['id']; ?>">
                            <i class="fas fa-heart"></i>
                        </button>
                    </div>
                </div>
                <div class="course-details">
                    <a href="<?php echo site_url('bundle_details/' . $my_bundle['id']) . '/' . rawurlencode(slugify($my_bundle['title'])); ?>"><h5 class="title"><?php echo $my_bundle['title']; ?></h5></a>
                    <div class="rating">
                        <?php
                        $total_rating =  $this->course_bundle_model->get_bundle_wise_ratings($my_bundle['id'])->row()->rating;
                        $number_of_ratings = $this->course_bundle_model->get_bundle_wise_ratings($my_bundle['id'])->num_rows();
                        if ($number_of_ratings > 0) {
                            $average_ceil_rating = ceil($total_rating / $number_of_ratings);
                        } else {
                            $average_ceil_rating = 0;
                    ...
                    <div class="d-block">
                        <div class="floating-user d-inline-block">
                            <?php $user_details = $this->user_model->get_all_user($my_bundle['user_id'])->row_array(); ?>
                            <img src="<?php echo $this->user_model->get_user_image_url($user_details['id']); ?>" width="30px" data-bs-toggle="tooltip" data-bs-placement="top" title="<?php echo $user_details['first_name'].' '.$user_details['last_name']; ?>"  onclick="return check_action(this,'<?php echo site_url('home/instructor_page/'.$user_details['id']); ?>');">
                        </div>
                            <p class="price text-right d-inline-block float-end"><small><?php echo currency($this->course_bundle_model->get_original_price_of_bundle($my_bundle['id'])); ?></small><?php echo currency($my_bundle['price']); ?></p>
                    </div>
                </div>
            </div>
        </div>
