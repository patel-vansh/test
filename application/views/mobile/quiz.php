...
<div id="quiz-stuffs" style="font-size: 20px;">
    <div id="quiz-body" class="text-center py-4">
        <div class="p-2" id="quiz-header">
            ...
            <?php if (count($quiz_questions->result_array()) > 0): ?>
                <button type="button" name="button" class="btn red mt-2" style="color: #fff; font-size: 20px;" onclick="getStarted(1)"><?php echo get_phrase("get_started"); ?></button>
            <?php endif; ?>
        </div>
            ...
            <?php if (count($quiz_questions->result_array()) > 0): ?>
                <?php foreach ($quiz_questions->result_array() as $key => $quiz_question):
                    ...
                    <div class="hidden" id = "question-number-<?php echo $key+1; ?>">
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="card text-start quiz-card">
                                    <div class="card-body">
                                        <h4 class="card-title"><strong><?php echo 'Q '.($key+1); ?> : <?php echo strip_tags($quiz_question['title']); ?></strong></h4>
                                    </div>
                                    <ul class="list-group list-group-flush">
                                        <?php if($quiz_question['type'] == "multiple_choice"): ?>
                                            <?php
                                            ...
                                            <?php endforeach; ?>
                                        <?php elseif($quiz_question['type'] == "single_choice"):  ?>
                                            <?php
                                                foreach ($options as $key2 => $option): ?>
                                                <li class="list-group-item quiz-options">
                                                    <div class="form-check text-start">
                                        		...
                                        		<div class="row justify-content-center">
                                        			...
                                        			<div class="col-md-9">
                                        				<div class="input-group mb-3">
                                        					<?php foreach($correct_answers as $key2 => $word): ?>
                                        						<span class="input-group-text"><?php echo ++$key2; ?></span>
                                        						<input type="text" onblur="enableNextButton('<?php echo $quiz_question['id']; ?>');" class="form-control" name="answer[]">
                                        					<?php endforeach; ?>
                                        				</div>
                                        			</div>
                                        		</div>
                        ...
                        <button type="button" name="button" class="btn red mt-2 mb-2" id = "next-btn-<?php echo $quiz_question['id'];?>" style="color: #fff; font-size: 20px;" <?php if(count($quiz_questions->result_array()) == $key+1):?>onclick="submitQuiz()"<?php else: ?>onclick="showNextQuestion('<?php echo $key+2; ?>')"<?php endif; ?> disabled><?php echo count($quiz_questions->result_array()) == $key+1 ? get_phrase("check_result") : get_phrase("submit_and_next"); ?></button>
                    </div>
                <?php endforeach; ?>
            <?php endif; ?>
        </form>
