...
<div class="row mb-4">
    <div class="col-lg-12">
        <div class="card text-left card-with-no-color-no-border">
            <div class="card-body">
                <h3 class="card-title"><img src="<?php echo $each['submitted_answer_status'] == 1 ? base_url('assets/frontend/default/img/green-tick.png') : base_url('assets/frontend/default/img/red-cross.png'); ?>" alt="" height="15px;"> <?php echo strip_tags(strval($question_details['title'])); ?></h3>
                <p class="card-text"><strong><?php echo get_phrase("your_answers"); ?>: </strong>
                    <?php
                ...
                <?php for ($i = 0; $i < count($correct_answers); $i++): ?>
                    <p class="card-text">
                        <strong><?php
                        if ($i == 0) {
                            echo get_phrase("correct_answers");
                        }?>: </strong>
                        <?php
                        if ($i == 0) {
                            echo  $options[($correct_answers[$i] - 1)];
                        } else {
                            echo str_repeat(" ", strlen(get_phrase("correct_answers"))) .  $options[($correct_answers[$i] - 1)];
                        }?>
