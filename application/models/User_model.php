...
class User_model extends CI_Model
{
    ...
    public function my_courses_trash($user_id = "")
    {
        if ($user_id == "") {
            $user_id = $this->session->userdata('user_id');
        }
        return $this->db->get_where('enrol_trash', array('user_id' => $user_id));
    }
    ...
    public function upload_user_image($image_code, $image = NULL, $show_errors = TRUE) {
        if ($image == NULL) $image = 'user_image';
        if (isset($_FILES[$image]) && $_FILES[$image]['name'] != "") {
            $config['upload_path'] = './uploads/user_image';  
            $config['allowed_types'] = 'jpg';
            $config['file_name'] = $image_code . '.jpg';
            ...
            if(!$this->upload->do_upload($image)) {
                ...
                $config['source_image'] = './uploads/user_image/' . $data["file_name"];  
                $config['create_thumb'] = FALSE;  
                $config['maintain_ratio'] = FALSE;  
                $config['quality'] = '60%';  
                $config['width'] = 200;  
                $config['height'] = 200;  
                $config['new_image'] = './uploads/user_image/' . $data["file_name"];  
