...
class Email_model extends CI_Model {
	...
	function password_reset_email_for_mobile($verification_code = '' , $email = '') {
		...
		if($query->num_rows() > 0) {
			...
			$email_data['message'] = 'You have requested a change of password from '.get_settings('system_name').'. Please change your new password from this link : <b style="cursor: pointer;"><u>'.site_url('login/change_password_for_mobile/').$verification_code.'</u></b><br><br><p>Please use this link in under 15 minutes.</p>';
	...
	public function course_purchase_notification($student_id = "", $payment_method = "", $amount_paid = ""){
		$purchased_courses 	= $this->crud_model->getCartItems($student_id);
	...
	public function bundle_purchase_notification($student_id = ""){
        $bundle_ids = $this->crud_model->getCartBundleItems($student_id);
