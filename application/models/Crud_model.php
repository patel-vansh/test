...
class Crud_model extends CI_Model {
    ...
    public function sort_question_paper_section($section_json) {
        ...
        foreach ($sections as $key => $value) {
            ...
            $this->db->update('paper_generator_sections', $updater);
        }
    }

    public function sort_question_paper_heading($heading_json) {
        $headings = json_decode($heading_json);
        foreach ($headings as $key => $value) {
            $updater = array(
                'sequence' => $key + 1
            );
            $this->db->where('id', $value);
            $this->db->update('paper_generator_headings', $updater);
        }
    }
    public function sort_question_paper_question($question_json) {
    ...
    public function get_question_paper_headings_of_section($section_id) {
        $this->db->order_by("sequence", "asc");
        return $this->db->get_where('paper_generator_headings', array('section_id' => $section_id));
    }

    public function get_question_paper_heading_by_id($heading_id) {
        return $this->db->get_where('paper_generator_headings', array('id' => $heading_id));
    }

    public function get_question_paper_sections($question_paper_id) {
        $this->db->where('id', $question_paper_id);
        return $this->db->get('paper_generator_question_paper');
    }

    public function get_question_paper_section_by_id($question_paper_section_id) {
        return $this->db->get_where('paper_generator_sections', array('id' => $question_paper_section_id));
    }

    public function get_standards_for_paper_generator($medium="", $board="") {
        if ($medium !== "") {
        ...
        $rows = $this->db->get('paper_generator_master')->result_array();
        $standards = array();
        foreach ($rows as $row) {
            if (!in_array($row['standard'], $standards)) {
    ...
    public function get_subjects_for_paper_generator($medium_id = "", $board_id = "") {
        if ($medium_id !== "")
            $this->db->where('medium_id', $medium_id);
        if ($board_id !== "")
            $this->db->where('board_id', $board_id);
        return $this->db->get('paper_generator_master');
    }

    public function get_question_paper_subjects_by_id($subject_id) {
        $this->db->where('id', $subject_id);
        return $this->db->get('paper_generator_master');
    }

    public function get_mediums_for_paper_generator($standard="", $board="", $return_as_object=false) {
        if ($standard !== "") {
    ...
    public function get_boards_for_paper_generator($standard="", $medium="") {
        if ($standard !== "") {
            $this->db->where('standard', $standard);
        }
        if ($medium !== "") {
    ...
    public function get_chapters_for_paper_generator($medium_id = "", $board_id = "", $subject_id = "") {
        if ($medium_id !== "")
            $this->db->where('medium_id', $medium_id);
        if ($board_id !== "")
            $this->db->where('board_id', $board_id);
        if ($subject_id !== "")
            $this->db->where('board_id', $subject_id);
        return $this->db->get('paper_generator_chapters');
    }

    public function get_question_papers_by_instructor($instructor_id = "") {
        if ($instructor_id == "") {
    ...
    public function add_question_paper() {
        ...
        $data['medium_id'] = $this->input->post('medium_id');
        $data['board_id'] = $this->input->post('board_id');
        $data['subject_id'] = $this->input->post('subject_id');
        $data['max_time'] = $this->input->post('max_time');
        $data['marks'] = $this->input->post('marks');
        $data['instructor_id'] = $this->session->userdata('user_id');
        $data['created_on'] = time() + 19800;

        $this->db->insert('paper_generator_question_paper', $data);

        $question_paper_id = $this->db->insert_id();
        $this->session->set_flashdata('flash_message', get_phrase('question_paper_has_been_added_successfully'));
        return $question_paper_id;
    }

    public function delete_question_paper($question_paper_id) {
        $this->db->where('id', $question_paper_id);
        $this->db->delete('paper_generator_question_paper');

        $this->db->where('question_paper_id', $question_paper_id);
        $this->db->delete('paper_generator_sections');

        $this->db->where('question_paper_id', $question_paper_id);
        $this->db->delete('paper_generator_headings');
    }

    public function add_question_paper_heading($section_id, $question_paper_id) {
        $headings = $this->db->get_where('paper_generator_headings', array('section_id' => $section_id, 'question_paper_id' => $question_paper_id))->result_array();
        $sequences = array();
        foreach ($headings as $heading) {
            $sequences[] = intval($heading['sequence']);
        }
        ...
        $data['question_paper_id'] = $question_paper_id;
        $data['sequence'] = $max_sequence;
        $this->db->insert('paper_generator_headings', $data);

        $heading_id = $this->db->insert_id();

        $this->db->where('id', $question_paper_id);
        $question_paper_details = $this->db->get('paper_generator_question_paper')->result_array()[0];
        $previous_headings_id = json_decode($question_paper_details['headings_id']);

        $updater = array();
        if (sizeof($previous_headings_id) > 0) {
            array_push($previous_headings_id, $heading_id);
            $updater['headings_id'] = json_encode($previous_headings_id);

        } else {
            $headings_id = array();
            array_push($headings_id, $heading_id);
            $updater['headings_id'] = json_encode($headings_id);
        }
        $this->db->where('id', $question_paper_id);
        $this->db->update('paper_generator_question_paper', $updater);
    }

    public function edit_question_paper_heading($heading_id) {
        $data['heading'] = html_escape($this->input->post('heading'));
        $data['marks'] = $this->input->post('marks');

        $this->db->where('id', $heading_id);
        $this->db->update('paper_generator_headings', $data);
    }

    public function delete_question_paper_heading($heading_id) {
        $this->db->where('id', $heading_id);
        $this->db->delete('paper_generator_headings');
    }

    public function add_question_paper_section($question_paper_id) {
        $sections = $this->db->get_where('paper_generator_sections', array('question_paper_id' => $question_paper_id))->result_array();
        $sequences = array();
        foreach ($sections as $section) {
            $sequences[] = intval($section['sequence']);
        }
        ...
        $data['name'] = html_escape($this->input->post('section_title'));
        $data['marks'] = $this->input->post('marks');
        $data['question_paper_id'] = $question_paper_id;
        $data['sequence'] = $max_sequence;
        $this->db->insert('paper_generator_sections', $data);

        $section_id = $this->db->insert_id();

        $this->db->where('id', $question_paper_id);
        $question_paper_details = $this->db->get('paper_generator_question_paper')->result_array()[0];
        $previous_sections_id = json_decode($question_paper_details['sections_id']);

        $updater = array();
        if (sizeof($previous_sections_id) > 0) {
            array_push($previous_sections_id, $section_id);
            $updater['sections_id'] = json_encode($previous_sections_id);

        } else {
            $sections_id = array();
            array_push($sections_id, $section_id);
            $updater['sections_id'] = json_encode($sections_id);
        }
        $this->db->where('id', $question_paper_id);
        $this->db->update('paper_generator_question_paper', $updater);
    }

    public function edit_question_paper_section($section_id) {
        $data['name'] = html_escape($this->input->post('section_title'));
        $data['marks'] = $this->input->post('marks');

        $this->db->where('id', $section_id);
        $this->db->update('paper_generator_sections', $data);
    }

    public function delete_question_paper_section($question_paper_id, $section_id) {
        $this->db->where('id', $section_id);
        $this->db->delete('paper_generator_sections');

        $question_paper_details = $this->db->get_where('paper_generator_question_paper', array('id' => $question_paper_id))->result_array()[0];
        $sections_id = json_decode($question_paper_details['sections_id']);
        unset($sections_id[array_search($section_id, $sections_id)]);
        if (!$sections_id) {
            $sections_id = null;
        } else {
            $sections_id = json_encode($sections_id);
        }
        $this->db->where('id', $question_paper_id);
        $this->db->update('paper_generator_question_paper', array('sections_id' => $sections_id));
    ...
    public function get_sub_category_count_home($param1 = 0) {
        if ($param1 > 0) {
    ...
    public function enrol_history($course_id = "") {
        if ($course_id > 0) {
            return $this->db->query("SELECT * FROM enrol WHERE course_id = $course_id UNION ALL SELECT * FROM enrol_trash WHERE course_id = $course_id ORDER BY date_added DESC");
        } else {
            return $this->db->query("SELECT * FROM enrol UNION ALL SELECT * FROM enrol_trash ORDER BY date_added DESC");
        }
    }

    public function enrol_history_by_user_id($user_id = "") {
        return $this->db->query("SELECT * FROM enrol WHERE user_id = $user_id UNION ALL SELECT * FROM enrol_trash WHERE user_id = $user_id ORDER BY date_added DESC");
    }

    public function all_enrolled_student() {
        return $this->db->query("SELECT DISTINCT user_id FROM enrol UNION DISTINCT SELECT DISTINCT user_id FROM enrol_trash");
    }

    public function enrol_history_by_date_range($timestamp_start = "", $timestamp_end = "") {
        $re = array();
        $enrol = $this->db->query("SELECT * FROM enrol WHERE date_added >= $timestamp_start AND date_added <= $timestamp_end ORDER BY date_added DESC")->result_array();
        $enrol_trash = $this->db->query("SELECT * FROM enrol_trash WHERE date_added >= $timestamp_start AND date_added <= $timestamp_end ORDER BY date_added DESC")->result_array();
        $bundle_payment = $this->db->query("SELECT * FROM bundle_payment WHERE date_added >= $timestamp_start AND date_added <= $timestamp_end ORDER BY date_added DESC")->result_array();
        $bundle_payment_trash = $this->db->query("SELECT * FROM bundle_payment_trash WHERE date_added >= $timestamp_start AND date_added <= $timestamp_end ORDER BY date_added DESC")->result_array();
        $res = array_merge($enrol, $enrol_trash, $bundle_payment, $bundle_payment_trash);
        $date_added = array();
        foreach ($res as $key => $value) {
            $date_added[$key] = $value['date_added'];
        }
        array_multisort($date_added, SORT_DESC, $res);
        $re['result_array'] = $res;
        return $re;
    }
    ...
    public function purchase_history($user_id = 0) {
        if ($user_id > 0 && $user_id != NULL) {
            $payment = $this->db->query("SELECT * FROM payment WHERE user_id = $user_id")->result_array();
            $bundle_payment = $this->db->query("SELECT * FROM bundle_payment WHERE user_id = $user_id")->result_array();
            $bundle_payment_trash = $this->db->query("SELECT * FROM bundle_payment_trash WHERE user_id = $user_id")->result_array();
            $res = array_merge($payment, $bundle_payment, $bundle_payment_trash);
            usort($res, function ($item1, $item2) {
                return $item1['date_added'] <=> $item2['date_added'];
            });
            return $res;
        } else {
            $payment = $this->db->query("SELECT * FROM payment")->result_array();
            $bundle_payment = $this->db->query("SELECT * FROM bundle_payment")->result_array();
            $bundle_payment_trash = $this->db->query("SELECT * FROM bundle_payment_trash")->result_array();
            $res = array_merge($payment, $bundle_payment, $bundle_payment_trash);
            usort($res, function ($item1, $item2) {
                return $item1['date_added'] <=> $item2['date_added'];
            });
    ...
    public function add_course($param1 = "") {
        ...
        $data['subscription_limit'] = $this->input->post('subscription_limit');
    ...
    public function get_course_bundle_by_id($bundle_id = "") {
        return $this->db->get_where('course_bundle', array('id' => $bundle_id));
    }
    ...
    public function add_lesson() {
        ...
        if ($lesson_type == 'video') {
            ...
            $data['video_type_for_mobile_application'] = $this->input->post('lesson_provider_for_mobile_application');
            ...
            $data['duration_for_mobile_application'] = $hour . ':' . $min . ':' . $sec;
            $data['video_type_for_mobile_application'] = $lesson_provider;
    ...
    public function update_frontend_settings() {
        ...
        $data['value'] = $this->input->post('insta');
    ...
    public function handleCartItem($course_id) {
        $cart = array();
        $user_details = $this->user_model->get_user($this->session->userdata('user_id'))->row_array();
        if ($user_details['cart_course'] == "") {
            array_push($cart, $course_id);
        } else {
            ...
            if (in_array($course_id, $cart)) {
                ...
                $cart = $container;
                // $key = array_search($course_id, $cart);
                // unset($cart[$key]);
            } else {
                array_push($cart, $course_id);
            }
    ...
    public function handleCartBundleItem($bundle_id) {
        $cart_bundle = array();
        $user_details = $this->user_model->get_user($this->session->userdata('user_id'))->row_array();
        if ($user_details['cart_bundle'] == "") {
            array_push($cart_bundle, $bundle_id);
        } else {
            $cart_bundle = json_decode($user_details['cart_bundle']);
            if (in_array($bundle_id, $cart_bundle)) {
                $container = array();
                ...
                // $key = array_search($bundle_id, $cart_bundle);
                // unset($cart[$key]);
            } else {
                array_push($cart_bundle, $bundle_id);
            }
    ...
    public function handleBundleWishList($bundle_id) {
        $wishlists = array();
        $user_details = $this->user_model->get_user($this->session->userdata('user_id'))->row_array();
        if ($user_details['wishlist_bundle'] == "") {
            array_push($wishlists, $bundle_id);
        } else {
            $wishlists = json_decode($user_details['wishlist_bundle']);
            if (in_array($bundle_id, $wishlists)) {
                $container = array();
                foreach ($wishlists as $key) {
                    if ($key != $bundle_id) {
                        array_push($container, $key);
                ...
                $wishlists = $container;
                // $key = array_search($bundle_id, $wishlists);
                // unset($wishlists[$key]);
            } else {
                array_push($wishlists, $bundle_id);
            }
        ...
        $updater['wishlist_bundle'] = json_encode($wishlists);
        $this->db->where('id', $this->session->userdata('user_id'));
        $this->db->update('users', $updater);
    }
    ...
    public function is_bundle_added_to_wishlist($bundle_id = "") {
        if ($this->session->userdata('user_login') == 1) {
            $wishlists = array();
            $user_details = $this->user_model->get_user($this->session->userdata('user_id'))->row_array();
            $wishlists = json_decode($user_details['wishlist_bundle']);
    ...
    public function is_added_to_cart($course_id = "") {
        if ($this->session->userdata('user_login') == 1) {
            $cart = array();
            $user_details = $this->user_model->get_user($this->session->userdata('user_id'))->row_array();
            $cart = json_decode($user_details['cart_course']);
    ...
    public function is_bundle_added_to_cart($bundle_id = "") {
        if ($this->session->userdata('user_login') == 1) {
            $cart_bundle = array();
            $user_details = $this->user_model->get_user($this->session->userdata('user_id'))->row_array();
            $cart_bundle = json_decode($user_details['cart_bundle']);
    ...
    public function getCartItems($user_id = "") {
        ...
        return json_decode($user_details['cart_course']);
    }

    public function getCartBundleItems($user_id = "") {
        if ($user_id == "") {
            $user_id = $this->session->userdata('user_id');
        }
        $user_details = $this->user_model->get_user($user_id)->row_array();
        return json_decode($user_details['cart_bundle']);
    }
    ...
    public function getBundleWishLists($user_id = "") {
        if ($user_id == "") {
            $user_id = $this->session->userdata('user_id');
        }
        $user_details = $this->user_model->get_user($user_id)->row_array();
        return json_decode($user_details['wishlist_bundle']);
    }
    ...
    public function enrol_student($user_id, $transaction_id) {
        $purchased_courses = $this->getCartItems($user_id);
    ...
    public function course_purchase($user_id, $method, $amount_paid, $param1 = "", $param2 = "", $coupon = "") {
        if ($method == "paytm")
            $coupon = $param2;
        $purchased_courses = $this->getCartItems($user_id);
    ...
    public function get_bundles_by_wishlists() {
        $wishlists = $this->getBundleWishLists();
        if (sizeof($wishlists) > 0) {
            $this->db->where_in('id', $wishlists);
            return $this->db->get('course_bundle')->result_array();
        } else {
            return array();
        }
    }
    ...
    public function get_bundles_of_wishlists_by_search_string($search_string) {
        $wishlists = $this->getBundleWishLists();
        if (sizeof($wishlists) > 0) {
            $this->db->where_in('id', $wishlists);
            $this->db->like('title', $search_string);
    ...
    ////////private message//////
    function get_messages_from_thread_code($message_thread_code) {
        $this->db->where("message_thread_code", $message_thread_code);
        return $this->db->get("message");
    }

    function read_thread_code_messages($message_thread_code, $current_user) {
    ...
    function enrol_to_free_course_mobile($course_id = "", $user_id = "") {
        ...
        if ($this->db->get_where('course', array('id' => $course_id))->row('is_free_course') == 1)
            if ($this->db->get_where('enrol', array('user_id' => $user_id, 'course_id' => $course_id))->num_rows() == 0)
                return $this->db->insert('enrol', $data);
            else
                return "already enrolled";
        else
            return "this course is not free";
    }
    ...
    public function get_discounted_price_after_applying_coupon($coupon_code) {
        ...
        foreach ($this->getCartBundleItems() as $cart_item) {
            $bundle_details = $this->crud_model->get_course_bundle_by_id($cart_item)->row_array();
