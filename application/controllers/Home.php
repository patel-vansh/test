...
class Home extends CI_Controller
{
    ...
    public function home()
    {
        ...
        $config = array();
        //$config = pagintaion($rows->num_rows(), 1);
        $config['base_url']  = site_url('course_bundles/');
        $this->pagination->initialize($config);

        $this->db->where('status', 1);
        $page_data['course_bundles'] = $this->db->get('course_bundle','', $this->uri->segment(2));
    ...
    public function my_wishlist()
    {
        ...
        $my_bundles = $this->crud_model->get_bundles_by_wishlists();
        $page_data['my_courses'] = $my_courses;
        $page_data['my_bundles'] = $my_bundles;
    ...
    public function purchase_history()
    {
        ...
        $total_rows = count($this->crud_model->purchase_history($this->session->userdata('user_id')));
        $config = array();
    ...
    public function handleBundleWishList($return_number = "")
    {
        if ($this->session->userdata('user_login') != 1) {
            redirect('login', 'refresh');
        } else {
            if (isset($_POST['bundle_id'])) {
                ...
                $this->crud_model->handleBundleWishList($bundle_id);
            }
            if ($return_number == 'true') {
                echo sizeof($this->crud_model->getBundleWishLists());
            } else {
                $this->load->view('frontend/' . get_frontend_settings('theme') . '/wishlist_items');
            }
        }
    ...
    public function handleCartItems($return_number = "")
    {
        if ($this->session->userdata('user_login') != 1) {
            ...
            if (isset($_POST['course_id'])) {
                $course_id = $this->input->post('course_id');
                $this->crud_model->handleCartItem($course_id);
            }
            if ($return_number == 'true') {
                echo sizeof($this->crud_model->getCartItems());
            } else {
    ...
    public function handleCartBundleItems($return_number = "")
    {
        if ($this->session->userdata('user_login') != 1) {
            redirect('login', 'refresh');
        } else {
            if (isset($_POST['bundle_id'])) {
                $bundle_id = $this->input->post('bundle_id');
                $this->crud_model->handleCartBundleItem($bundle_id);
            }
            if ($return_number == 'true') {
                echo sizeof($this->crud_model->getCartBundleItems());
            } else {
                $this->load->view('frontend/' . get_frontend_settings('theme') . '/cart_items');
            }
        }
    ...
    public function handleCartItemForBuyNowButton()
    {
        if ($this->session->userdata('user_login') != 1) {
            ...
            $course_id = $this->input->post('course_id');
            $previous_cart_items = $this->crud_model->getCartItems();
            if ($previous_cart_items != null && $previous_cart_items != "") {
                if (!$this->crud_model->is_added_to_cart($course_id)) {
                    array_push($previous_cart_items, $course_id);
                }
            } else {
                $previous_cart_items = array();
                array_push($previous_cart_items, $course_id);
            }
            $updater['cart_course'] = json_encode($previous_cart_items);
            $this->db->where('id', $this->session->userdata('user_id'));
    ...
    public function handleCartBundleItemForBuyNowButton()
    {
        if ($this->session->userdata('user_login') != 1) {
            redirect('login', 'refresh');
        } else {
            $bundle_id = $this->input->post('bundle_id');
            $previous_cart_bundle_items = $this->crud_model->getCartBundleItems();
            if ($previous_cart_bundle_items != null && $previous_cart_bundle_items != "") {
                if (!$this->crud_model->is_bundle_added_to_cart($bundle_id)) {
                    array_push($previous_cart_bundle_items, $bundle_id);
                }
            } else {
                $previous_cart_bundle_items = array();
                array_push($previous_cart_bundle_items, $bundle_id);
            }
            $updater['cart_bundle'] = json_encode($previous_cart_bundle_items);
            $this->db->where('id', $this->session->userdata('user_id'));
            $this->db->update('users', $updater);
            $this->load->view('frontend/' . get_frontend_settings('theme') . '/cart_items');
        }
    ...
    public function get_my_wishlists_by_search_string()
    {
        ...
        $bundle_details = $this->crud_model->get_bundles_of_wishlists_by_search_string($search_string);
        $page_data['my_courses'] = $course_details;
        $page_data['my_bundles'] = $bundle_details;
    ...
    {
        if ($this->session->userdata('user_login') == 1) {
            $this->crud_model->enrol_to_free_course_mobile($course_id, $this->session->userdata('user_id'));
