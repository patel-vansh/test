...
class User extends CI_Controller {
    ...
    public function ajax_sort_question_paper_section() {
        $section_json = $this->input->post('itemJSON');
        $this->crud_model->sort_question_paper_section($section_json);
    }

    public function ajax_sort_question_paper_heading() {
        $heading_json = $this->input->post('itemJSON');
        $this->crud_model->sort_question_paper_heading($heading_json);
    }

    public function question_paper_sections($question_paper_id = "", $function_type = "", $section_id = "") {
        if ($this->session->userdata('user_login') != true) {
            redirect(site_url('login'), 'refresh');
        }

        if ($function_type == 'add') {
            $this->is_the_question_paper_belongs_to_current_instructor($question_paper_id);
            $this->crud_model->add_question_paper_section($question_paper_id);
            $this->session->set_flashdata('flash_message', get_phrase('section_has_been_added_successfully'));
        } elseif ($function_type == 'edit') {
            $this->is_the_question_paper_belongs_to_current_instructor($question_paper_id);
            $this->crud_model->edit_question_paper_section($section_id);
            $this->session->set_flashdata('flash_message', get_phrase('section_has_been_updated_successfully'));
        } elseif ($function_type == 'delete') {
            $this->is_the_question_paper_belongs_to_current_instructor($question_paper_id);
            $this->crud_model->delete_question_paper_section($question_paper_id, $section_id);
            $this->session->set_flashdata('flash_message', get_phrase('section_has_been_deleted_successfully'));
        }
        redirect(site_url('user/question_paper/edit/' . $question_paper_id));
    }

    public function question_paper_headings($section_id = "", $question_paper_id = "", $function_type = "", $heading_id = "") {
        if ($this->session->userdata('user_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        if ($function_type == 'add') {
            $this->is_the_question_paper_belongs_to_current_instructor($question_paper_id);
            $this->crud_model->add_question_paper_heading($section_id, $question_paper_id);
            $this->session->set_flashdata('flash_message', get_phrase('heading_has_been_added_successfully'));
        } elseif ($function_type == 'edit') {
            $this->is_the_question_paper_belongs_to_current_instructor($question_paper_id);
            $this->crud_model->edit_question_paper_heading($heading_id);
            $this->session->set_flashdata('flash_message', get_phrase('heading_has_been_updated_successfully'));
        } elseif ($function_type == 'delete') {
            $this->is_the_question_paper_belongs_to_current_instructor($question_paper_id);
            $this->crud_model->delete_question_paper_heading($heading_id);
            $this->session->set_flashdata('flash_message', get_phrase('heading_has_been_deleted_successfully'));
        }
        redirect(site_url('user/question_paper/edit/' . $question_paper_id));
    }
    ...
    public function question_papers() {
        $page_data['page_name'] = 'question_papers';
        $page_data['page_title'] = get_phrase('question_papers');
        $page_data['question_papers'] = $this->crud_model->get_question_papers_by_instructor();
        $this->load->view('backend/index.php', $page_data);
    }

    public function question_bank() {
        $page_data['page_name'] = 'question_bank';
        $page_data['standards'] = $this->crud_model->get_standards_for_paper_generator();
        $page_data['mediums'] = $this->crud_model->get_mediums_for_paper_generator();
        $page_data['boards'] = $this->crud_model->get_boards_for_paper_generator();
        $page_data['page_title'] = get_phrase('question_bank');
        $this->load->view('backend/index.php', $page_data);
    }

    public function get_standards_for_question_bank() {
        $medium = $_GET['medium'] !== "All" ? $_GET['medium'] : "";
        $board = $_GET['board'] !== "All" ? $_GET['board'] : "";
        echo json_encode($this->crud_model->get_standards_for_paper_generator($medium, $board));
    }

    public function get_mediums_for_question_bank() {
        $standard = $_GET['standard'] !== "All" ? $_GET['standard'] : "";
        $board = $_GET['board'] !== "All" ? $_GET['board'] : "";
        echo json_encode($this->crud_model->get_mediums_for_paper_generator($standard, $board));
    }

    public function get_boards_for_question_bank() {
        $standard = $_GET['standard'] !== "All" ? $_GET['standard'] : "";
        $medium = $_GET['medium'] !== "All" ? $_GET['medium'] : "";
        echo json_encode($this->crud_model->get_boards_for_paper_generator($standard, $medium));
    }

    public function get_mediums_from_standard() {
        $standard = $this->input->get("standard");
        $mediums = $this->crud_model->get_mediums_for_paper_generator($standard, "", true);
        echo json_encode($mediums);
    }

    public function get_boards_from_medium() {
        $medium_id = $this->input->get("medium_id");
        $boards = $this->crud_model->get_boards_for_paper_generator($medium_id)->result_array();
        echo json_encode($boards);
    }

    public function get_subjects_from_board() {
        $medium_id = $this->input->get("medium_id");
        $board_id = $this->input->get("board_id");
        $subjects = $this->crud_model->get_subjects_for_paper_generator($medium_id, $board_id)->result_array();
        echo json_encode($subjects);
    }

    function is_the_question_paper_belongs_to_current_instructor($question_paper_id) {
        $is_valid = 1;
        $question_paper = $this->crud_model->get_question_paper_by_id($question_paper_id)->result_array();
        if ($question_paper[0]['instructor_id'] !== strval($this->session->userdata('user_id'))) {
            $this->session->set_flashdata('error_message', get_phrase('you_do_not_have_right_to_access_this_question_paper'));
            $is_valid = 0;
            redirect(site_url('user/question_papers'), 'refresh');
        }
        return $is_valid;
    }
    ...
    public function question_paper_actions($param1 = "", $param2 = "") {
        ...
        if ($param1 == "add") {
            $question_paper_id = $this->crud_model->add_question_paper();
            echo site_url('user/question_paper/edit/' . $question_paper_id);
        } elseif ($param1 == "edit") {
            $this->is_the_question_paper_belongs_to_current_instructor($param2);
            $this->crud_model->update_question_paper($param2);
            redirect(site_url('user/question_paper/edit/' . $param2));
    ...
    public function question_paper($param1 = "", $param2 = "") {
        ...
        if ($param1 == 'add') {
            ...
            $page_data['question_paper_id'] = $param2;
            $page_data['page_title'] = get_phrase('edit_question_paper');
            $page_data['sections_id'] = json_decode($this->crud_model->get_question_paper_sections($param2)->result_array()[0]['sections_id']);
            $this->load->view('backend/index', $page_data);
        }
    }
